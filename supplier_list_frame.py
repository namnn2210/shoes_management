...
class SupplierListFrame(tk.Frame):
    def __init__(self, master):
        ...
        self.master = master
        self.master.title("Hệ thống quản lí nhà cung cấp")
        ...
        self.add_supplier_button = tk.Button(
            self, text="Thêm nhà cung cấp", command=self.open_add_supplier_window)
    ...
    def open_edit_supplier_window(self, supplier_data):
        ...
        edit_window.title("Cập nhật nhà cung cấp")
        ...
        save_button = tk.Button(button_frame, text="Lưu", command=lambda: self.save_edited_supplier(
            supplier_data[0], [entry.get() for entry in entries], edit_window))
        save_button.pack(side=tk.LEFT)

        # Create a "Delete" button within the button frame
        delete_button = tk.Button(
            button_frame, text="Xóa", command=lambda: self.delete_supplier(supplier_data[0], edit_window))
        delete_button.pack(side=tk.LEFT)

    def save_edited_supplier(self, supplier_id, updated_data, edit_window):
        try:
            update_supplier(supplier_id, *updated_data)
            messagebox.showinfo("Result", "Cập nhật nhà cung cấp thành công")
    ...
    def delete_supplier(self, supplier_id, edit_window):
        try:
            result = messagebox.askyesno(
                "Xác nhận xóa", "Bạn có chắc chắn muốn xóa nhà cung cấp nhà?")
            if result:
                delete_supplier(supplier_id)
                messagebox.showinfo("Result", "Xóa nhà cung cấp thành công")
                # Refresh the table data
                self.refresh_table_data()
    ...
    def open_add_supplier_window(self):
        ...
        add_supplier_window.title("Thêm nhà cung cấp")
        add_supplier_window.geometry("600x400")
        ...
        submit_button = tk.Button(
            add_supplier_window, text="Lưu", command=lambda: self.add_new_supplier(add_supplier_window))
        submit_button.pack()
    ...
    def add_new_supplier(self, add_supplier_window):
        try:
            ...
            messagebox.showinfo("Result", "Thêm nhà cung cấp thành công")
            # Refresh the table data
            self.refresh_table_data()
